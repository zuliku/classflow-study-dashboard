...
/** 桌面展开断点（≥1280）：用户可手动折叠；<1280 强制 icon rail */
...
/**
 * Sidebar（App Chrome V2）：
 * - <768：隐藏（Bottom Nav）
 * - 768–1279：强制 icon rail（w-16）
 * - ≥1280：用户可折叠（展开 224px / 折叠 64px；持久化 sidebarCollapsed）
 * 结构：Navigation area（flex-1 / min-h-0 / overflow-y-auto）+ Profile region（shrink-0）。
 */
export function Sidebar() {
  ...
  const itemAlign = effectiveCollapsed ? "justify-center" : "justify-start";

  return (
    <aside
      className={cn(
        "hidden md:flex h-screen bg-[#F7F5F5] border-r border-line flex-col p-3.5 sticky top-0 select-none z-20 shrink-0",
    ...
    >
      ...
      <div className="flex-1 min-h-0 overflow-y-auto space-y-3 scrollbar-none">
        {/* Brand Logo：折叠显示图形 Mark；展开显示全横版 Logo（Responsive Swap，非缩放） */}
        <div
        ...
        >
          {showLabels ? (
            <img
              src="/logo.png"
              alt="ClassFlow"
              className="w-9 h-9 md:w-auto md:h-auto md:max-w-[180px] object-contain mix-blend-multiply"
            />
          ) : (
            <img
          ...
          {MAIN_NAV_ITEMS.map((item) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "relative w-full flex items-center gap-2.5 px-2 py-2 rounded-xl text-xs font-medium transition-colors duration-[var(--motion-base)] ease-[var(--ease-standard)] group text-left",
                  itemAlign,
                  showLabels && "px-3",
                  isActive
              ...
              >
                ...
                <span
                  ...
                  className={cn("truncate", showLabels ? "inline" : "hidden")}
                >
                  {item.label}
                </span>

                {/* Rail Tooltip（折叠态）：hover / focus 均可见 */}
                {effectiveCollapsed && (
                  <span
              ...
              >
                ...
                <span
                  className={cn(
                    ...
                    "flex items-center gap-2.5 text-xs font-semibold transition-colors duration-[var(--motion-base)]",
                    itemAlign,
                    showLabels && "px-3",
          ...
          {GLOBAL_NAV_ACTIONS.map((item) => {
            ...
            return (
              ...
              >
                ...
                {/* unread 小圆点（fired && !readAt；不显示数字）：折叠居中 / 展开右上 */}
                {isReminders && hasUnread && (
                  <span
                    aria-hidden="true"
                    data-testid="reminder-unread-dot"
                    className={cn(
                      "absolute top-1.5 w-2 h-2 rounded-full bg-danger border border-surface",
                      effectiveCollapsed
                        ? "left-1/2 -translate-x-1/2"
                        : "left-auto right-2.5 translate-x-0"
                    )}
                  />
                )}
                <span
