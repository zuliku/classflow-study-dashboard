/**
 * Kiro Sidecar 位置逻辑（Move V1）：top/right 表示 + clamp + position-aware resize。
 * 纯函数（无 DOM），供 Shell 与测试共用。
 * - 保存 top/right（与既有 top-6/right-6 + left-resize 保持 right edge 语义一致）
 * - clamp：panel 四边至少保留 SIDECAR_VIEWPORT_MARGIN（24px）
...
/** 持久化值 hydrate 归一（localStorage 可能为任意值；无 viewport 时只做结构归一） */
...
/** 位置 clamp：四边均保留 SIDECAR_VIEWPORT_MARGIN（窗口过小时 Math.max 兜底，不产生负几何） */
