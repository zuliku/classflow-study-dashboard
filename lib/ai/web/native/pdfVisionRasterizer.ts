...
export async function rasterizeWebPdfPages(
  ...
  if (!render && !deps?.skipLoad) {
    try {
      ...
      const desktopFontUrl = (process.env.CLASSFLOW_PDF_FONTS_URL ?? "").trim();
      const nodeInit =
        typeof window === "undefined"
          ? // 变量 + 模板字符串（非字面量）：webpack 不做静态解析（与 attachments/pdf.ts 一致），运行时解析为文件 URL；
            // 桌面版由主进程注入 CLASSFLOW_PDF_FONTS_URL（asar unpack 路径），dev 回落到 import.meta.url
            { standardFontDataUrl: desktopFontUrl || new URL(`${fontDir}`, import.meta.url).toString() }
          : {};
