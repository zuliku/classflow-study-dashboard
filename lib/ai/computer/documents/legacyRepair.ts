/**
 * Legacy Kiro DOCX Bounded Repair（V2.6）。
 *
 * 只修两种已在真实用户文件中确认的错误（不扩展任何其它 surgery）：
 *
 ...
 *
 * 实现约束：只操作已知 producer 的确定性结构，绝不动 runs 内部内容。
 */
...
export async function repairLegacyKiroDocx(bytes: Uint8Array): Promise<LegacyRepairResult> {
  ...
  let changed = false;
  if (documentXml && before.directTableRuns > 0) {
    const repaired = repairDocumentXml(documentXml);
