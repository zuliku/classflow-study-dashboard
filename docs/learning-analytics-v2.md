...
> Status: 实现完成（Analytics Engine + History Projection + Period Comparison + Learning Signals + UI V2）
...
- `components/analytics/`：`LearningAnalyticsView`（头部 + range selector + coverage 提示 + 4 指标卡 + 趋势图 + 信号卡 + 课程投入 + 专注节奏 + 执行情况）
...
## 八、Kiro History Tools

`lib/ai/tools/read/history.ts` 不变（query 默认 30d / hard 90d / ≤200 条；summarize 默认 28d / hard 366d；浏览器 IndexedDB 直连）。Analytics V2 **不新增任何 AI 调用**。
