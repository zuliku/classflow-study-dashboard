...
describe("agent mode defaults", () => {
  ...
  it("guided：read/create allow，modify/move ask", () => {
  ...
  it("workspace-auto：read/create/modify allow，move ask", () => {
    ...
    expect(prep("workspace-auto", "fs.move", { rootId: "output", path: "a.md" }).effect).toBe("ask");
  });
});

describe("hard deny", () => {
  it("V1 hard deny 无视 agent mode", () => {
    for (const mode of ["plan", "guided", "workspace-auto"] as KiroAgentMode[]) {
      for (const cap of ["fs.delete", "app.open", "app.reveal", "shell.execute", "network.access"] as const) {
        expect(prep(mode, cap, { rootId: "output", path: "a.md" }).effect).toBe("deny");
      }
    }
  });

  it("read-only root：mutation hard deny（任何 mode）", () => {
    for (const mode of ["plan", "guided", "workspace-auto"] as KiroAgentMode[]) {
