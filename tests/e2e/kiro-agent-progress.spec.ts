...
/**
 * V4/V4.1：leading progress 首次到达即进入 Worklog（commentary 立即可见）→
 * Tool 到达只追加 Tool row（commentary lane 不变）→ begin_final_answer（server execute）
 * + Final Answer 同一响应 → 完成（Markdown stable、Worklog 完成不折叠）。
 * 全程无时间启发，只有协议决定通道。
 */
test("Agent 流程：leading progress 首次到达即进入 Worklog，后续 Tool 不改变其 lane（无跨 lane 闪现）", async ({ page }) => {
  ...
  const sse = await startSseServer((bodyJson) => {
    ...
    // 请求 2（V4.1）：begin_final_answer 由 server execute，Final Answer 同一响应继续
    //（不再需要单独的 boundary → Browser → 新请求；request 数 3 → 2）
    return [
      {
        events: [
          JSON.stringify({ type: "start", messageId: "mock-agent-1" }),
      ...
      { delay: 80, events: [JSON.stringify({ type: "start-step" }), JSON.stringify({ type: "text-start", id: "final-text" })] },
  ...
  await expect.poll(() => requestCount).toBe(2); // V4.1：boundary server execute + Final 同一响应（3 → 2）
  ...
  // T4：完成 → Markdown stable（h2 真实渲染）、cursor 消失；
  // V4.1：完成瞬间 Worklog 不自动折叠（无 layout 跳动），commentary 保持可见
  ...
  await expect(worklog.locator('[data-state="open"]').first()).toBeVisible(); // 完成不折叠（layout stability）
  await expect(commentary).toBeVisible();
 ...
 */
test("普通聊天（V4.1）：boundary + Final Answer 同一请求（requestCount === 1，无 Worklog）", async ({ page }) => {
  let requestCount = 0;
  const sse = await startSseServer((bodyJson) => {
    requestCount += 1;
    // V4.1：begin_final_answer server execute + Final Answer 在同一响应（direct answer 单请求）
    return [
      {
        events: [
      ...
      { delay: 60, events: [JSON.stringify({ type: "start-step" }), JSON.stringify({ type: "text-start", id: "plain-text" })] },
  ...
  // V4.1：boundary + Final 同一请求 → requestCount === 1（direct answer 不再两个请求）
  await expect(msg.locator(".kiro-markdown").first()).toContainText("这是普通回答的", { timeout: 5000 });
  await expect.poll(() => requestCount).toBe(1);
  // V4.1：direct answer 不渲染 Worklog（0 business tool + 0 commentary；无 milestone / 无"已完成 0 个步骤"）
  await expect(page.getByTestId("kiro-worklog")).toHaveCount(0);
  await expect(page.getByText("已完成 · 0 个步骤")).toHaveCount(0);
 ...
 * T5: Tool 2 done
 * T6+T7: begin_final_answer（server execute）+ Final Answer 同一响应（无 milestone）
 *
...
test("V4：Progressive Worklog 分阶段真实到达（T0 时 progress 已在 Worklog）", async ({ page }) => {
  ...
  // T6+T7（V4.1）：begin_final_answer（server execute）+ Final Answer 同一响应；
  // 无 milestone（worklog 只含 commentary + tool）；无入场动画 class
  await expect(msg.locator(".kiro-markdown").first()).toContainText("正在整理", { timeout: 10000 });
  await expect(page.getByTestId("kiro-streaming-cursor")).toBeVisible();
  await expect(page.getByTestId("kiro-worklog-milestone")).toHaveCount(0);
  await expect(page.locator(".kiro-worklog-enter")).toHaveCount(0);

  // 收尾（V4.1）：summary 显示已完成 · 2 个步骤；完成瞬间 Worklog 不自动折叠（layout stability）
